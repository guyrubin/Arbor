...
/**
 * COACH-3 — the CapabilityRegistry the app boots with is real wiring, not
 * scaffolding: the google-TTS adapter and the config's structured-text
 * adapters (Gemini/Claude on Vertex, or the dev Gemini key) are registered at
 * startup, and lookups stay exact-match fail-closed.
 */
...
describe("buildCapabilityRegistry", () => {
  ...
  it("registers both Vertex structured-text adapters (Claude for coach, Gemini for analysis)", () => {
    const registry = buildCapabilityRegistry(createTestConfig(), stubModelProvider);
    expect(registry.get("structured_text", "vertex_claude").provider.model).toBe("claude-3-5-sonnet@anthropic");
