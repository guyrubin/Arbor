...
export default function DailyPlayCard({
  ...
  const why =
    reason === "concern-match"
      ? `Picked because ${childName} has had some ${activity.domain} moments lately.`
      : `A good fit for where ${childName} is right now.`;
    ...
    >
      <div className="p-6">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <span className="inline-flex items-center gap-1.5 text-[13px] font-bold" style={{ color: GREEN }}>
              <Sprout className="w-3.5 h-3.5" /> Play idea for today
            </span>
        ...
        <p className="text-[15px] leading-relaxed mt-3" style={{ color: INK, textWrap: "pretty" } as React.CSSProperties}>
          <span style={{ color: GREEN, fontWeight: 700 }}>Builds: </span>{activity.whatItBuilds}
        </p>
        ...
        <div className="flex flex-wrap items-center gap-2.5 mt-5">
          ...
          >
            <Check className="w-4 h-4" /> {done ? `Added to ${childName}'s day` : "We did this"}
          </button>
          <button
            ...
            style={{ background: GREEN_SOFT, color: GREEN }}
          >
            <MessageSquare className="w-4 h-4" /> Coach me on this
          </button>
        </div>
