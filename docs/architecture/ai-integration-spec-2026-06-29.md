...
Framed in the ROS capability-optimization format (P0 = highest leverage). These are recommendations, not commitments.
...
**Theme:** the three P0/P1 voice items convert a strong silent brain into a product that *sounds* as premium as it *is* — the single biggest "more engaging" lever. The eval gate is what makes "more effective" measurable rather than asserted.

---

## 7. The one decision

**Approve a "give Arbor a real voice" workstream — P0 neural-TTS seam + default-on Gemini Live — as the next Arbor AI epic?** Everything else (avatar voice persona, eval gate, media persistence, SoapBox) sequences behind that yes. The seam pattern already exists (`childAsr.ts`), the safety screen already gates pre-playback, and Live is already coded — this is wiring, not invention.
