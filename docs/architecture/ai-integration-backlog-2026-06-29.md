...
> **Current decision update — 2026-07-22:** The provider-specific recommendation in this historical backlog is superseded by the **eval-selected capability fabric** below. Full market and journey rationale: `PAI/projects/arbor/ARBOR-AI-MARKET-AND-ARCHITECTURE-2026-07-22.md`. GPT-Live is a future adapter target until its API ships; GPT-Realtime-2 and Gemini Live are current bake-off candidates. Existing safety, consent, storage, clinical, and production gates remain binding.

### AR-AI 2026-07-22 execution backlog

| ID | P | Build item | User value | Acceptance criteria | Gate |
|---|:--:|---|---|---|---|
| **AR-AI-01** | P0 | Add capability, provider-adapter, policy-decision, route-policy, and normalized usage/error contracts around `modelRouter` | Better models can be adopted without journey rewrites | Existing behavior passes; route/UI code names capabilities, not vendors; fake-adapter contract tests cover text, stream, realtime, ASR, TTS, vision/image | none |
| **AR-AI-02** | P0 | Separate policy eligibility from quality/cost routing | Child data never falls silently to weaker processing terms | Consent, audience, locale, data class, region, retention, and risk filter candidates before scoring; cross-policy fallback fails closed; route kill switch | DPA/DPIA per provider |
| **AR-AI-03** | P0 | Generalize realtime session; remove hard-coded Gemini voice/journey coupling | Arbor can use the best voice and always has a floor | Gemini, OpenAI API, and browser/SSE adapters; captions, interruption, stop, timeout, normalized events; no-dead-air fallback | AR-AI-01/02 |
| **AR-AI-04** | P0 | Versioned EN/HE blind voice bake-off | Provider choice reflects real Arbor moments | ≥50 scripted + 20 consented internal/pilot sessions; TTFA, interruption, completion, safety, HE naturalness, cost/useful-action; pinned versions; no prod child data | eval harness; native HE review |
| **AR-AI-05** | P0 | AI receipt + route telemetry | Debuggable, evidence-led AI without leaking content | Store capability/journey/provider/model/version, context classes, citations, action types, safety result, latency/cost; exclude raw audio, prompts, child free text | REL-2 / FD-6 |
| **AR-AI-06** | P0 | Shared answer contract: do now, say this, watch for, optional why | A parent gets a usable step, not a transcript wall | Voice/visual share schema; ≤1 clarification by default; every span screened before speech; escalation overrides normal flow; EN/HE tests | clinical firewall |
| **AR-AI-07** | P0 | Post-session review: save moment, plan, reminder, professional question | Conversation becomes progress, never silent automation | Explicit confirm; edit/discard; duplicate-safe; provenance retained; session→confirmed-action metric | AR-AI-06 |
| **AR-AI-08** | P0 | Close voice/photo/text Capture → confirm → memory → outcome | Parents stop repeating context; Arbor learns confirmed truth | One draft-signal schema; uncertainty visible; confirm required; later outcome prompt; correction/removal changes future context | child-data guards |
| **AR-AI-09** | P1 | In-context memory/evidence inspector | Parent can understand and correct why Arbor said this | Reviewed sources + approved memory; correct/exclude; no raw system prompt; a11y/RTL | RAG/citations; AR-AI-08 |
| **AR-AI-10** | P1 | Route scorecards, shadow eval, canary promotion/rollback | Models improve safely over time | Fixtures/consented traffic only; challenger output never shown; threshold+margin promotion; rollback on safety/SLO; human gate for new processor/path | AR-AI-04/05 |
| **AR-AI-11** | P1 | Continuity-of-care pack from confirmed facts, actions, outcomes | Parent brings a coherent story to a professional | Editable/removable; facts vs inference; sources/time window; degrade to brief; parent approves; never auto-send | clinical review |
| **AR-AI-12** | P2 | Parent-started 3–5 minute live vision co-play in an existing scene | Personalized play without an open-ended AI relationship | On-air + STOP; fresh consent; no-store test; parent summary; audio-only fallback | child voice/vision gate |
| **AR-AI-13** | P2 | Child voice product/safety controls | Natural voice without surveillance or dependency | No friend/human impersonation, always-listening, emotion/diagnosis inference, or cloning; age/session limits; parent controls; kill switch | clinical/privacy/AI Act |

**Order:** 01 → 02 → 03+05 → 04 → 06 → 07; 08 → 09; then 10 → 11; child-facing 12 → 13 last. **Primary metric:** voice session → confirmed useful action or appropriate escalation. Naturalness alone is not completion.

**Execution status — 2026-07-22:** AR-AI-01 foundation is **started** in `app/src/ai/capabilities/` (vendor-neutral contracts, exact adapter registry, barrel export, offline tests). AR-AI-02 is **started** with eligibility-before-scoring policy gates for capability, audience, data class, region, customer-data training, and retention; no production route has been migrated yet, so current Claude/Gemini behavior is unchanged. The `/api/tts` trust boundary was also hardened: caller-provided text is re-screened immediately before synthesis and unsafe text returns a safe 422 without calling the audio provider. UI/journey files were intentionally untouched because a parallel session owns them.
