...
## 3. The IA — ten hubs, every route homed

> **Amended 2026-08-27 (§5 amendment 2 of 2).** The heading read "nine" while the table below listed ten and the shipped code declares the ten-hub architecture (`app/src/lib/navigation.ts`: "the TEN-hub Arbor information architecture"; `surfaceContract.ts` HUB_IDS). Ten is correct — the count is now consistent across doc, table and code.
...
| **H5 — Delight + reach** | 12–16 | M-Wave-5: celebration chain (PrideMomentCard/CelebrationMoment) · wow→story seed · teach-empty-states. Native shell (Capacitor) beta with kid-lock re-penetration on native back · ~~store prep~~ **[superseded 2026-08-27 — see §5 amendment 1: the store track ran in weeks 1–2, not 12–16; native IAP, age-hard parental gate, account deletion and the kid-egress hardening are merged and live]** · MON-1 design · I-02 brand ("one tree, two worlds"). | D30 trending to ≥40; NPS ≥50; store-ready builds green. **Pre-declared cut line: H5 delight first, then M4.4/M4.6 — never H0–H2.** |
